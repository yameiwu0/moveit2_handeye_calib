...
/* 加载关节位置数组等信息 */
bool cubicSpline::loadData(const double *x_data, const double *y_data, int count, double bound1, double bound2, BoundType type)
{
...
void cubicSpline::Interpolation(const std::vector<double> &time_from_start,
                                ...
                                std::vector<double> &a_out)
{
    ...
    if (!loadData(time_from_start.data(), positions.data(),
                  positions.size(), 0, 0, cubicSpline::BoundType_Second_Derivative))
    {
        return;
    }
    ...
    time_after_spline.clear();
    while (x_out < max_time)
    {
        x_out += rate;
        getYbyX(x_out, y_out, vel, acc);
        p_out.push_back(y_out);
        v_out.push_back(vel);
        a_out.push_back(acc);
        time_after_spline.push_back(x_out);
    }
}
//
void cubicSpline::Traj_Generate(const std::shared_ptr<const control_msgs::action::FollowJointTrajectory_Goal> &goal,
                                ...
                                std::vector<double> &time_list)
{
    ...
    time_after_spline.clear();
    RCLCPP_INFO(rclcpp::get_logger("cubic_spline"), "First Move_group give us %d points", point_num);
    // ------------------- 主体函数 -------------------
    if (point_num > 3)
    {
        std::vector<std::vector<double>> p_input(dof, std::vector<double>(point_num));

        std::vector<std::vector<double>> v_input(dof, std::vector<double>(point_num));
        std::vector<double> time_from_start(point_num);
        for (int i = 0; i < point_num; i++)
        {
            for (int j = 0; j < dof; j++)
            {
                p_input[j][i] = goal->trajectory.points[i].positions[j];
                v_input[j][i] = goal->trajectory.points[i].velocities[j];
            }
            time_from_start[i] =
                goal->trajectory.points[i].time_from_start.sec +
                goal->trajectory.points[i].time_from_start.nanosec / 1e9;
        }
        double max_time = time_from_start.back();
        RCLCPP_INFO(rclcpp::get_logger("cubic_spline"), "Second Move_group max_time is: %f", max_time);
        // 循环插值所有关节
        for (int j = 0; j < dof; j++)
        {
            Interpolation(time_from_start, p_input[j], rate, max_time,
                          p_joint_[j], v_joint_[j], a_joint_[j]);
        }
    }
    else if (point_num > 0)
    {
        ...
        for (int i = 0; i < point_num; i++)
        {
            for (int j = 0; j < dof; j++)
            {
                p_joint_[j].push_back(goal->trajectory.points[i].positions[j]);
                v_joint_[j].push_back(goal->trajectory.points[i].velocities[j]);
                a_joint_[j].push_back(0.0); // 原始点里没加速度，可以补0
            }
            double t = goal->trajectory.points[i].time_from_start.sec +
                   goal->trajectory.points[i].time_from_start.nanosec / 1e9;
            time_after_spline.push_back(t);
        }
        
    }
    positions_list.resize(time_after_spline.size(), std::vector<double>(dof, 0.0));
    velocity_list.resize(time_after_spline.size(), std::vector<double>(dof, 0.0));
    for (size_t i = 0; i < time_after_spline.size(); i++)
    {
        for (int j = 0; j < dof; j++)
        {
            positions_list[i][j] = p_joint_[j][i];
            velocity_list[i][j] = v_joint_[j][i];
        }
    }
    if (positions_list.empty())
    {
        RCLCPP_WARN(rclcpp::get_logger("cubic_spline3"), "positions_list is empty, cannot generate trajectory");
        return;
    }
    time_list = time_after_spline;
}
