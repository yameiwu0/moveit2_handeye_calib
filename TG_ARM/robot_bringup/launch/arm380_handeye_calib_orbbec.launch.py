#!/usr/bin/env python3
"""TG_ARM ARM380 手眼标定启动文件 - Orbbec Astra 相机版 (Eye-in-Hand 配置)"""
...
def generate_launch_description():
    ...
    # 默认 rviz 配置文件路径 (Orbbec 版本)
    default_rviz_config = os.path.join(
        get_package_share_directory('robot_bringup'),
        'rviz', 'arm380_handeye_calib_orbbec.rviz'
    )
    ...
    # 将 Orbbec 的 camera_link 连接到机械臂 Link6
    camera_mount_tf = Node(
        ...
        arguments=[
            ...
            "--yaw", "0.0",
            "--frame-id", "Link6",
            "--child-frame-id", "camera_link",  # Orbbec 的 camera_link
        ],
    )
