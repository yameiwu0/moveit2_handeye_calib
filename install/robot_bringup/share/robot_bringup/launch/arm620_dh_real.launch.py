...
def generate_launch_description():
    ...
    robot_description = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(get_package_share_directory('robot_description'), 'launch', 'arm620_dh_display.launch.py'))
    )
    ...
    robot_state_machine = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(get_package_share_directory('robot_state_machine'), 'launch', 'robot_state_arm620.launch.py'))
    )

    robot_gravity = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(get_package_share_directory('robot_dynamics'), 'launch', 'gravity_arm620_dh.launch.py'))
    )

    traj_smooth = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(get_package_share_directory('traj_smoother_py'), 'launch', 'traj_smoother.launch.py'))
    )
